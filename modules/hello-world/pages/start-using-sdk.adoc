...
:page-partial:
:lang: C++
:example-source: devguide:example$cxx/src/start_using.cxx
...
// https://github.com/couchbase/docs-sdk-cxx/tree/release/1.4/modules/devguide/examples/scala
//
// It can be seen built at
// https://docs.couchbase.com/cxx-sdk/1.4/hello-world/start-using-sdk.html
...
+
--
If you haven't already got a cluster set up, the easiest route is to https://cloud.couchbase.com/sign-up[sign up to Couchbase Capella and deploy a free tier operational cluster^],
...
https://github.com/cpm-cmake/CPM.cmake[CPM.cmake] is the  recommended way to include the library in your project.
...
[source,{example-source-lang}]
----
include::{example-source}[indent=0,tag=connect_capella]
----
...
[source,{example-source-lang}]
----
include::{example-source}[indent=0,tag=connect_onprem]
----
--
...
returning a  `document_exists` error --
...
If the collection does not have a document with this ID, the `get` method also returns a `document_not_found` error.
