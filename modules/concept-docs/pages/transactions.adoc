...
----
include::{example-source}[tag=create-simple,indent=0]
----
...
If the transaction is not retried then it will return a `couchbase::error` wrapping an `{transaction-failed}` error code, and its `cause()` method can be used for more details on the failure.

// TODO: Investigate how this can be achieved in C++. This should be possible with custom error codes, but could be improved. We might have an improvement related to this in a future release.
// The application can use this to signal why it triggered a rollback, by returning custom error codes, as so:
//
// [source,scala]
// ----
...
[source,{example-source-lang}]
----
include::{example-source}[tag=custom-metadata,indent=0]
----
...
[source,{example-source-lang}]
----
include::{example-source}[tag=custom-metadata-per,indent=0]
----
