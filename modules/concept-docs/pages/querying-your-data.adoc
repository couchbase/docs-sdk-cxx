...
// "Before CB {version-common} if a Index+QUERY node gets failed due to any infra issue, then it will not automatic failover."
...
* xref:{version-common}@server:connectors:intro.adoc[Connectors]
...
xref:{version-common}@server:learn:services-and-indexes/services/analytics-service.adoc[Couchbase Analytics Service (CBAS)] performs well on huge datasets, with complex aggregations, and uses {sqlpp} for Analytics, which gives a similar query experience to {sqlpp} for Query.
...
In particular, care should be taken over building indexes, to stop them becoming unnecessarily large -- see our xref:{version-common}@server:fts:full-text-intro.adoc[FTS documentation].
...
From Couchbase Server 6.5, xref:{version-common}@server:n1ql:n1ql-language-reference/searchfun.adoc[Search Functions] allow the use of FTS _within_ {sqlpp} queries.
