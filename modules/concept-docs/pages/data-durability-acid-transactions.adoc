...
:anyreplica-api: https://docs.couchbase.com/sdk-api/couchbase-java-client/com/couchbase/client/java/Collection.html#getAnyReplica(java.lang.String)
...
----
include::{example-source}[indent=0,tag=remove_with_durability]
----
...
=== Local Availability Zone Reads

IMPORTANT: Zone Aware Replica Reads are only accessible with the {name-sdk} working with Couchbase Server 7.6.2 or newer (Capella or self-managed).
...
The {name-sdk}, when making read replica requests, can be zone aware --
and set to always read from a copy of the document in the local zone.
...
In any case, Couchbase does not recommend this feature where read consistency is critical.
...
* Setting three replicas for the bucket disables durability for sync writes, also affecting transactions.
====
