...
See the xref:howtos:concurrent-async-apis.adoc[Async & Reactive APIs page] for practical examples.

== Error Handling

Best practices for error handling in {cpp} depend somewhat upon your choice of API:
future, or callback based, as covered in the xref:concurrent-async-apis.adoc[async and reactive API guide].
That guide also covers how errors are actually returned and handled.
Your application should always check if an operation returned an error, and potentially handle any specific error codes that may have returned.
