...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=imports,indent=0]
----

.Retrieve Sub-Document value
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=get,indent=0]
----

.Check existence of Sub-Document path
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=exists,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=get-callback,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=insert,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=mutation-combine,indent=0]
----

NOTE: `mutate_in` is an _atomic_ operation.  If any single operation fails, then the entire document is left unchanged.
...
The `array_prepend` and `array_append` operations are true array prepend and append operations.
Unlike fulldoc `append`/`prepend` operations (which simply concatenate bytes to the existing value), `array_append` and `array_prepend` are JSON-aware:

[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-append,indent=0]
----

[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-prepend,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-create,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-upsert,indent=0]
----


== Arrays as Unique Sets

Limited support also exists for treating arrays like unique sets, using the `array_add_unique` command.
This will do a check to determine if the given value exists or not before actually adding the item to the array:

[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-unique,indent=0]
----

Note that currently the `array_add_unique` will fail with a `couchbase::errc::key_value::path_mismatch` error code if the array contains JSON _objects_, or _arrays_.
The `array_add_unique` operation will also fail with `couchbase::errc::key_value::value_invalid` if the value to be added is one of those types as well.
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=array-insert,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=counter-inc,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=counter-dec,indent=0]
----
...
In this case, the `create_path` option may be used.

[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=create-path,indent=0]
----
...
Couchbase Server 7.6 and later support Sub-Doc xref:concept-docs:data-durability-acid-transactions.adoc#reading-from-replicas[lookup from replicas].
With 7.6.2 onwards supporting xref:concept-docs:data-durability-acid-transactions.adoc#preferred-server-group-replica-reads[reading from preferred Server Group].
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=any-replica,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=all-replicas,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=concurrent,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=cas,indent=0]
----


== Durability

Couchbase's xref:2.7@java-sdk::durability.adoc[traditional durability], using `persist_to` and `replicate_to`, is
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=durability,indent=0]
----
...
[source,c++]
----
include::devguide:example$cxx/src/subdocument.cxx[tag=sync-durability,indent=0]
----


== Error Handling

Sub-Document operations have their own set of error codes.
When programming with Sub-Document, be prepared for any of the full-document error codes (such as `couchbase::errc::key_value::document_not_found`) as well as
special Sub-Document errors which are received when certain constraints are not satisfied.
Some of the error codes include (all under the `couchbase::errc::key_value` namespace):

* `path_not_found`: When retrieving a path, this means the path does not exist in the document.
When inserting or upserting a path, this means the _immediate parent_ does not exist.
* `path_exists`: In the context of an _insert_, it means the given path already exists.
In the context of _arrayAddUnique_, it means the given value already exists.
* `path_mismatch`: This means the path may exist in the document, but that there is a type conflict between the path in the document and the path in the command.
...
* `path_invalid`: This means the path is invalid for the command.
