...
:lang: C++
:fts-example-source: devguide:example$cxx/src/search.cxx
:query-example-source: devguide:example$cxx/src/queries.cxx
...
// 
// It can be seen built at https://docs.couchbase.com/cxx-sdk//howtos/vector-searching-with-sdk.html
...
----
include::{query-example-source}[indent=0,tag=vector_search_with_query_hyperscale_index]
----
...
[source,{example-source-lang}]
----
include::{query-example-source}[indent=0,tag=vector_search_with_query_parameterized]
----
...
This first example performs a single vector query:
[source,{example-source-lang}]
----
include::{fts-example-source}[indent=0,tag=vector_search]
----

The example creates a `search_request`. 
This can contain a traditional Search query `search_query`, or the new `vector_search`, or both.
This example just uses the latter.

The `vector_search` allows you to perform one or more `vector_query`s.
...
Finally we execute the `search_request` against the Search index "vector-index", which has previously been setup to vector index the "vector_field" field.

This happens to be a scoped index hence just use, `scope.search()`.
If it was a global index, use `cluster.search()` instead - see <<Scoped vs Global Indexes>>.
...
Note: The `num_candidates` sets how many similar vectors are returned. 
...
[source,{example-source-lang}]
----
include::{fts-example-source}[indent=0,tag=multiple_vector_queries]
----

How the results are combined (AND or OR) is controlled with `vector_search_options.query_combination()`.
...
[source,{example-source-lang}]
----
include::{fts-example-source}[indent=0,tag=combined_search_request]
----
...
* Vector GSI Query is just like any other query from the SDK to the Query service: https://docs.couchbase.com/sdk-api/couchbase-cxx-client/classcouchbase_1_1vector__query.html[{cpp} SDK API Reference].
...
* Vector Search in the https://docs.couchbase.com/sdk-api/couchbase-cxx-client/classcouchbase_1_1vector__search.html[{cpp} SDK API reference].
