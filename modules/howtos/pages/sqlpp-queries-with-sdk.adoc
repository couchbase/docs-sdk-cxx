...
The returned `query_result` contains an `rows_as<S,T>` method, allowing the results to be converted into something useful.  The above
example demonstrates returning the results as a `tao::json::value` using the serializer `couchbase::codec::tao_json_serializer`, that uses the _taoJSON_ library.
